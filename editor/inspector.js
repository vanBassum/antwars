...
export class Inspector {
  constructor(el, selection, { terrainTool } = {}) {
    this._el          = el;
  ...
  _render(go) {
    ...
    const isTerrain = this._terrainTool && go === this._terrainTool._terrain;
    ...
    this._el.innerHTML = `
      ...
      <div class="inspector-body">
        ...
        ${isTerrain ? this._terrainSection() : ''}
      </div>`;

    this._el.querySelectorAll('.insp-input').forEach(input => {
      input.addEventListener('input', () => this._applyTransform(input));
    });

    if (isTerrain) this._wireTerrainSection();
  }
  ...
  _terrainSection() {
    ...
    return `
      <hr class="panel-separator">
      <div class="inspector-group-title">Terrain</div>
      <div class="field-row"><label>Seed</label>
        <input type="number" id="ti-seed" value="${s.seed}" step="1"></div>
      <div class="field-row"><label>Width</label>
        <input type="number" id="ti-width" value="${s.width}" step="8" min="16" max="512"></div>
      <div class="field-row"><label>Depth</label>
        <input type="number" id="ti-depth" value="${s.depth}" step="8" min="16" max="512"></div>
      <div class="field-row"><label>Height Scale</label>
        <input type="number" id="ti-hscale" value="${s.heightScale}" step="0.5" min="1"></div>
      <button class="panel-btn" id="ti-regen" style="margin-top:8px">Regenerate</button>`;
  }
  ...
  _wireTerrainSection() {
    this._el.querySelector('#ti-regen')?.addEventListener('click', () => {
      const settings = {
        ...
        width:       parseInt(this._el.querySelector('#ti-width').value)  || 128,
        depth:       parseInt(this._el.querySelector('#ti-depth').value)  || 128,
        heightScale: parseFloat(this._el.querySelector('#ti-hscale').value) || 5,
      };
      this._terrainTool.loadSettings(settings);
